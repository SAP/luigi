...
#Run acutal test
(sleep $TIME; set -e && trap "echo killWebserver executed SIGTERM; exit 0" SIGTERM && cypress run --env configFile=setuptest.json,url=$TESTURL --browser chrome -c video=false && killWebserver $PORT) & (
curl -s $URL > ./setup.sh &&
