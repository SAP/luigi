...
const DEFAULT_LOCALE = assetParameters.locales.default;let initialized = false;
...
};const isSlot = el => el && el instanceof HTMLElement && el.localName === "slot";let suf;
let rulesObj = {
	include: [/^ui5-/],
	exclude: [],
};
 ...
 * render to and update a container.
 */
const svg = (strings, ...values) => new SVGTemplateResult(strings, values, 'svg', defaultTemplateProcessor);const cache = new Map();
...
const scopedSvg = (strings, ...values) => svg(scopeHTML(strings, tags, suffix), ...values);const KeyCodes = {
	BACKSPACE: 8,
	TAB: 9,
	ENTER: 13,
	SHIFT: 16,
...
const block1 = (context) => { return scopedHtml`<ui5-icon style="${styleMap(context.styles.icon)}" class="ui5-button-icon" name="${ifDefined(context.icon)}" show-tooltip=${ifDefined(context.iconOnly)}></ui5-icon>`; };
...
Icon.define();const BUTTON_ARIA_TYPE_ACCEPT = {key: "BUTTON_ARIA_TYPE_ACCEPT", defaultText: "Positive Action"};const BUTTON_ARIA_TYPE_REJECT = {key: "BUTTON_ARIA_TYPE_REJECT", defaultText: "Negative Action"};const BUTTON_ARIA_TYPE_EMPHASIZED = {key: "BUTTON_ARIA_TYPE_EMPHASIZED", defaultText: "Emphasized"};registerThemeProperties("@ui5/webcomponents-theme-base", "sap_fiori_3", defaultThemeBase);
registerThemeProperties("@ui5/webcomponents", "sap_fiori_3", defaultTheme);
...
const metadata$2 = {
	...
	slots: /** @lends sap.ui.webcomponents.main.Button.prototype */ {
		 ...
		 * <br><br>
		 * <b>Note:</b> Ðlthough this slot accepts HTML Elements, it is strongly recommended that you only use text in order to preserve the intended design.
		 *
		 * @type {Node[]}
		 * @slot
		 * @public
