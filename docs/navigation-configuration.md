...
- **loadingIndicator.enabled** shows a loading indicator when you switch between micro front-ends. Super fast micro front-ends might want to disable this feature. The **loadingIndicator.loadingIndicator** is enabled by default.
- **loadingIndicator.hideAutomatically** disables the automatic hiding of the loading indicator. Use `LuigiClient.uxManager().hideLoadingIndicator()` to hide it manually in your micro front-end during the startup. The **loadingIndicator.hideAutomatically** is enabled by default.
