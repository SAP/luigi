...
- **isolateViewFallback** renders all views in new frames. This setting overrides the same-domain frame re-usage. The **isolateViewFallback** is disabled by default and it can be overwritten by the **isolateView** value on a single node level.
