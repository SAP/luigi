...
- **userInfoFn** provides a function to get user information. It returns a promise of a **userinfo** object which contains user `name` and/or `email` to display in the profile dropdown menu. Optinal you can set the attribute `picture` to display user's profile image in the top navigation.
