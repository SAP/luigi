...
- `onAuthExpireSoon`**[function](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/function)** is executed before token will expire. For oAuth2 and OIDC it is possible to specify the seconds when the event will be fired by setting **accessTokenExpiringNotificationTime**.