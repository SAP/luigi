...
- **badgeCounter** adds a badge with a number and a label to a node. Nodes that are part of a category show a cumulated number of all badges in this category. **badgeCounter** is only available for top navigation items.
  ...
  Gets updated when you click the navigation. Use `Luigi.navigation().updateTopNavigation()` in Luigi Core or execute `LuigiClient.sendCustomMessage({id: 'luigi.navigation.update-badge-counters'});` from within a micro frontend to trigger a manual update.
