...
- **onNodeActivation** is an optional function that is executed when a request to navigate to the node occurs. The function receives as input parameter the node object as described in the configuration. This function can return synchronously or asynchronously. If the function returns boolean `false`, the navigation is not triggered, otherwise navigation happens as usual.
