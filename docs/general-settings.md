...
**thirdPartyCookieCheck** is an Object which expects a function called `thirdPartyCookieMessage` and ether `thirdPartyCookiesScriptLocation` or `thirdPartyCookieDetection`.
  * **thirdPartyCookieMessage** as a function where you could call an alert like `Luigi.ux().showAlert({})`.
