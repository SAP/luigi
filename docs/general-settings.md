...
- **type**: function
- **description**: called on web component creation, it gives you full control over the created web component DOM element. You can modify it to your needs just before it is added to the DOM tree.
This function is called with these parameters:
* `wc` is the web component DOM element. It is not yet added to the DOM tree, but all attributes are already set.
* `currentNode` is the navigation node
* `extendedContext` extended settings about the current node
* `nodeId` `id`, which is set on the web component object in your config or if not defined a generaded `id` from Luigi.
* `isSpecialMf` you can specify if the webcomponent is rendered in a modal, splitview or drawer. `false` by default.
