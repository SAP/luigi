...
async function symbolicLinkAll() {
    for (let i = 0; i < symbolicLinkList.length; i++) {
      await createSymbolicLinkFromTo(symbolicLinkList[i].source, symbolicLinkList[i].destination, symbolicLinkList[i].scope, i, symbolicLinkList.length);
    }
  }
...
async function createSymbolicLinkFromTo(source, destination, scope, index, numLinks) {
    try {
        ...
        console.log(`\x1b[32m[${index+1}/${numLinks}]: Symbolic link created successfully.\x1b[0m : \x1b[33m${sourcePath} \x1b[0m =>  \x1b[35m${linkFolderPath}\x1b[0m`);
    } catch (error) {
